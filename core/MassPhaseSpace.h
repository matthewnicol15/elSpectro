...
double wee = 0;
while (true) {
  wee = PhaseSpaceWeight(parentM);

  // If the envelope is too small, grow it and retry acceptance test
  if (wee > max) {
    max = wee * 1.05;     // 5% headroom
    continue;
  }

  if (wee >= gRandom->Uniform() * max * _suppressPhaseSpace)
    break;
}

// no warning print needed anymore
_successN++;
    }
