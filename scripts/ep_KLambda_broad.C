...
void ep_KLambda_broad(double ebeamE, int nEvents, int fileno)
{
  ...
  auto Lambda = static_cast<DecayingParticle *>(particle(3122, model(new GenericModelst{dist1, {}, {2212, -211}})));

  // decay of pGamma* to Lambda + K
  ...
  writer(new LundWriter{Form("/home/nics/work/York/elSpectro/scripts/outputs/ep_to_LambdaK_%d_part%d.dat", (int)ebeamE, fileno)});
  ...
  TFile *fout = TFile::Open(Form("/home/nics/work/York/elSpectro/scripts/outputs/ep_to_LambdaK_%d_part%d.root", (int)ebeamE, fileno), "recreate");
